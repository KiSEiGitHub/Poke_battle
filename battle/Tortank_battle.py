...
def tortank_battle(
    ...
    while True:
        ...
        Tortank_stat.pv -= dmg_dealt

        if Tortank_stat.pv < 0:
            print(f"{player_name} et son {player_pokemon} ont gagné")
            break

        rival_dmg = rival_attack('Tortank', player_pokemon, Tortank_stat.lvl, Tortank_stat.attaque,
                     Tortank_attack_damage.Hydrocanon, Tortank_attack_damage.Virbraqua,
                     Tortank_attack_damage.Ebullition, Tortank_attack_damage.Surf, deff)
        ...
        print(f"Tortank: {round(Tortank_stat.pv)} hp")
